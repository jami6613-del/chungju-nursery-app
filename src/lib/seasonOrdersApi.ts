...
export async function migrateLegacyLocalToSupabase(
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : "이관 중 오류";
...
export interface SeasonInventoryRow {
  boardIndex: number;
  variety: string;
  stock_pan: number;
  deducted_pan: number;
  stock_pogi: number;
  deducted_pogi: number;
}

export async function fetchSeasonInventory(boardIndex: number): Promise<SeasonInventoryRow[]> {
  const { data } = await supabase
    .from("season_inventory")
    .select("*")
    .eq("board_index", boardIndex);
  const rows = (data as Record<string, unknown>[]) ?? [];
  return rows.map((r) => ({
    boardIndex: r.board_index as number,
    variety: (r.variety as string) ?? "",
    stock_pan: Number(r.stock_pan ?? 0) || 0,
    deducted_pan: Number(r.deducted_pan ?? 0) || 0,
    stock_pogi: Number(r.stock_pogi ?? 0) || 0,
    deducted_pogi: Number(r.deducted_pogi ?? 0) || 0,
  }));
}

export async function upsertSeasonInventory(
  boardIndex: number,
  rows: Array<Omit<SeasonInventoryRow, "boardIndex">>,
  userId?: string,
): Promise<void> {
  ...
  await supabase.from("season_inventory").upsert(payload, { onConflict: "board_index,variety" });
}
